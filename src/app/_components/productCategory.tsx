...
import { getproductCategories, getUserCategories, updateUserCategory,createUserCategory } from "../action";

export const ProductCategory = () => {
  const [productCategoryList, setProductCategoryList] = useState<any[]>([]);
  const [userProductCategoryObj, setUserProductCategoryObj] = useState<{ [key: string]: any }>({
    userId: '',
    categoryId: '',
    active: ''
  })
  ...
  const updateUserCategories = async (productCategoryObj: {categoryId: string, isSelected: boolean}) => {
    let userAuth = localStorage.getItem("userAuth");
    if (userAuth) {
      let parsedUserAuth: { userId?: string } | null = JSON.parse(userAuth);
      if(userProductCategoryObj[productCategoryObj.categoryId]){
        ...
        await createUserCategory({categoryId: productCategoryObj.categoryId,userId: parsedUserAuth?.userId ?? "",})
      }
  ...
  return (
    <div className="flex h-full w-full items-center justify-center">
      <div className="flex min-h-[70vh] min-w-[40vw] flex-col items-center justify-center gap-12 rounded-[20px] border-2 border-[#C1C1C1]">
        ...
        <h4>We will keep you notified.</h4>
        <div>
